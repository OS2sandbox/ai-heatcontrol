📆 _sidst opdateret: {{ site.time | date: '%B %d, %Y' }}_

# Referat :   OS2 AI Heat Control - projektgruppemøde 18.08.2026

**Status**
- [x] Mødet kalendersat
- [ ] Dagsorden udarbejdet
- [ ] Referat påbegyndt
- [ ] Referat sendt til godkendelse
- [ ] Referat godkendt
- [ ] Referat publiceret  


## Mødefakta
  
**Mødenavn**: OS2 AI Heat Control projektgruppemøde  
**Tid**: 18.08.2026  kl. 10.00 - 12.00  
**Sted**: Teams

#### Deltagere:
- [ ] Mikkel Groot Andersen, Silkeborg Kommune, IT
- [ ] Martin Hansen, Kolding Kommune
- [ ] Jakob Hovgaard Kaiser, Aarhus Kommune
- [ ] Anders Aschlund Bach Burkahl, Ikast-Brande Kommune
- [ ] Villads Laraignou Mønsted, Gladsaxe Kommune (deltog på Teams)
- [ ] Jesper Buchhardt, Silkeborg Kommune, EJD
- [ ] Jan Maack Kjerbye, OS2-sekretariatet
- [ ] Jakob Thøtt Nørby, 4BC
- [ ] Anna-Lis Berg, OS2-sekretariatet

#### Afbud:
- 

#### Faciliteret af:
- [ ] Jakob Thøtt Nørby, 4BC
  
#### Observers/ Gæster:
- [ ] 

## Møde Indhold

### Møde beskrivelse

Formålet med mødet er at afklare projektets videre rammer med fokus på fremtidig organisering, opfølgning på budget og udbudsproces. Mødet skal skabe fælles retning for den næste fase af OS2 AI Heat Control og afklare beslutninger, ansvar og næste skridt.

### Baggrundsmateriale ((fx præsentation, styringsredskaber, mm)
- [Issues](https://github.com/OS2sandbox/ai-heatcontrol/issues?q=is%3Aissue%20state%3Aopen%20label%3APM)
- Budgetopfølgning (fremsendes særskilt på mail)

### Dagsorden
1. Formalia     
   1. Verifikation af deltagere, mødeleder og referent
   2. Godkendelse af referat fra tidligere møde og dagsorden
2. Opfølgning:
   1. Status på igangværende aktiviteter (se [Issues]([https://github.com/OS2sandbox/ai-heatcontrol/issues](https://github.com/OS2sandbox/ai-heatcontrol/issues?q=state%3Aopen%20label%3APM)))
3. Tema 1: Fremtidig organisering
4. Tema 2: Opfølgning på budget
5. Tema 3: Udbudsproces
6. Evt. & næste møde

#### Beslutnings- og actionlogs
- [Link til strategisk beslutningslog](https://github.com/OS2sandbox/ai-heatcontrol/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22SDR%22) 
- [Link til architectural decision records](https://github.com/OS2sandbox/ai-heatcontrol/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22ADR%22) 
- link til risikolog (ikke tilgængelig pt)

## Møde referat

### AD 1: Formalia
   - [ ] Mødeleder og referent udpeget
   - [ ] Referat godkendt

### AD 2: Opfølgning
 - [ ] **Status på igangværende aktiviteter**  
    1. Actions fra PM (https://github.com/OS2sandbox/ai-heatcontrol/issues?q=state%3Aopen%20label%3APM)
      a) 

### AD 3: Tema 1: **Fremtidig organisering**

### AD 4: Tema 2: **Opfølgning på budget**

### AD 5: Tema 3: **Udbudsproces**

### AD 6: Evt. & næste møde 

 
  ---
